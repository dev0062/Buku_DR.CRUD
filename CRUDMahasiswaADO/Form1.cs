...
namespace CRUDMahasiswaADO
{
    public partial class Form1 : Form
    {
        ...
        private void FormMahasiwa_Load(Object sender, EventArgs e)
        {
            //ComboBOx JK Manual
        ...
        private void btnCari_Click(object sender, EventArgs e)
        {
            try
            {
                // 1. Cek apakah NIM sudah diisi sebelum mencari
                if (string.IsNullOrWhiteSpace(txtNIM.Text))
                {
                    MessageBox.Show("Silakan masukkan NIM yang ingin dicari terlebih dahulu!", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtNIM.Focus();
                    return;
                }

                // 2. Panggil fungsi GetMhsByNIM dari class DAL
                DataTable dtHasilCari = dbLogic.GetMhsByNIM(txtNIM.Text);

                // 3. Cek apakah datanya ada di database
                if (dtHasilCari != null && dtHasilCari.Rows.Count > 0)
                {
                    // Jika ketemu, tampilkan hanya data tersebut ke DataGridView
                    bindingSource.DataSource = dtHasilCari;
                    dataGridView1.DataSource = bindingSource;

                    // Atur ulang layout gambar agar rapi
                    DataGridViewImageColumn fotoColumn = (DataGridViewImageColumn)dataGridView1.Columns["Foto"];
                    fotoColumn.ImageLayout = DataGridViewImageCellLayout.Stretch;
                ...
                {
                    // Jika tidak ketemu, beritahu user dan tampilkan kembali semua data
                    MessageBox.Show("Data mahasiswa dengan NIM tersebut tidak ditemukan.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ClearForm();
                    LoadData();
                }
            }
